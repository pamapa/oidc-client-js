...
export class ResponseValidator {
    ...
    _processCode(state: SigninState, response: SigninResponse): Promise<SigninResponse> {
        ...
        return this._tokenClient.exchangeCode(request).then(tokenResponse => {
            ...
            /*TODO: port-TS for(var key in tokenResponse) {
    ...
    _validateIdTokenAttributes(state: SigninState, response: SigninResponse) {
    ...
    _validateIdTokenAndAccessToken(state: SigninState, response: SigninResponse) {
        return this._validateIdToken(state, response).then(response => {
            return this._validateAccessToken(response);
        });
    }

    _getSigningKeyForJwt(jwt: any) {
    ...
    _getSigningKeyForJwtWithSingleRetry(jwt: any) {
